...
class TokenManager:
    ...
    def initiate_auth_flow(self) -> str:
        ...
        # Check for and fix duplicate state parameters
        if auth_url.count('state=') > 1:
            logger.warning("Detected duplicate state parameters, fixing...")
            from urllib.parse import urlparse, parse_qs, urlencode, urlunparse

            parsed = urlparse(auth_url)
            params = parse_qs(parsed.query, keep_blank_values=True)

            # Keep only the first state parameter
            if 'state' in params and len(params['state']) > 1:
                logger.info(f"Found {len(params['state'])} state parameters, keeping first one")
                ...
                logger.info("Fixed duplicate state parameters")
