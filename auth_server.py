...
class AuthServer:
    ...
    def start(self) -> str:
        ...
        # Extract the state parameter from the OAuth URL for validation
        from urllib.parse import urlparse, parse_qs
        parsed = urlparse(auth_url)
        params = parse_qs(parsed.query)
        if 'state' in params and params['state']:
            self.state = params['state'][0]  # Use the first (and should be only) state value
            logger.info(f"Using OAuth state parameter: {self.state}")
        else:
            logger.warning("No state parameter found in OAuth URL")
            self.state = None
